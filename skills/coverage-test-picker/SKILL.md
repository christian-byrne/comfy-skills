...
For each candidate in the top 20, determine test type by path. **Default to unit/component tests** — they run faster, are easier to write, and provide better isolation. Only recommend E2E when the file's value is primarily in its integration with other systems (routing, full page flows, canvas interactions).

- **Unit test**: paths containing `composables/`, `stores/`, `utils/`, `services/`, `schemas/`, `scripts/` (non-Vue pure logic)
- **Component test** (Vitest + Vue Test Utils): `.vue` files in `components/`, `views/`, or `renderer/` — test reactive behavior, props, emits, and store interactions via `shallowMount`. This is the **default for Vue components** unless the component's value is purely in browser-level interactions.
- **E2E test** (Playwright): Only when testing cross-component flows, canvas rendering, real browser APIs (drag-and-drop, file upload, WebGL), or full-page user journeys. The task file should explain why E2E is necessary over a component test.
...
Pick N candidates randomly from the top 20 (default N=5):

```bash
# Default: pick 5. User can override.
N=${PICK_COUNT:-5}
...
For **each** picked candidate, create a task file `test-task-<filename>.md`. This file will be moved to the worktree root in Phase 4. It MUST start with `<!-- DO NOT COMMIT THIS FILE -->` on the first line.
...
- `pnpm test:unit` may fail if `nx` is not installed — use `npx vitest run <path>` as fallback
- `onMounted`/`onUnmounted` hooks do NOT fire in bare composable tests — wrap in a `defineComponent` + `mount()` from `@vue/test-utils`
- The `vue/one-component-per-file` lint rule will flag test wrapper components — extract the wrapper to a single reusable `const Wrapper = defineComponent(...)` per `describe` block
- `happy-dom` doesn't support `capture: true` event propagation — test keyboard/pointer logic via direct handler calls or use `jsdom` environment override
- Worktrees may not have `node_modules` — run `pnpm install` in the worktree before running tests

## Completion Workflow

After writing the tests, the agent MUST:

1. Run quality gates: `pnpm typecheck && pnpm lint && pnpm format`
2. Run the tests: `npx vitest run <test-file-path>`
3. Commit: `git add <test-file> && git commit -m "test: add unit tests for <name>"`
4. Handoff to code review (use `code-review` skill or oracle self-review)
5. Address any review feedback
6. Push and create PR: `git push origin <branch>` then `gh pr create --title "test: add unit tests for <name>" --body "..."`
...
# 3. MOVE (not copy!) the task file to worktree root
mv "test-task-$(basename "$SOURCE_FILE" .ts).md" "$WT_PATH/"
```
...
Manual steps:
  ...
  2. mv test-task-<name>.md worktrees/test-<name>/
  3. cd worktrees/test-<name> && <start your agent with the task file>
```
...
- ❌ **Putting task files in subdirectories** — Place at worktree root, not `temp/plans/` or any nested path
- ❌ **Picking only 1 candidate** — Default to 5 candidates unless the user specifies otherwise
- ❌ **Forgetting DO NOT COMMIT** — Every task file must start with `<!-- DO NOT COMMIT THIS FILE -->`
