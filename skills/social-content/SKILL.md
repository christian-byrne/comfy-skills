---
name: social-content
description: 'Social media content strategy and creation — engaging content that builds audience, drives engagement, and supports business goals across platforms. Use when creating social media content, planning a social content calendar, adapting content per platform, writing posts to grow audience or engagement, or aligning social content with marketing goals.'
...
If you already have a product marketing context doc (positioning, audience, brand voice), read it before asking questions. Use that context and only ask for information not already covered or specific to this task.
...
### Anti-Slop Check for Social Posts

Social content is the highest-risk surface for AI tells — readers spot them instantly. Run these checks (full ruleset: `avoid-ai-writing` skill):

- **Dramatic fragmentation** — "[Word]. That's it." / "X. And Y. And Z." → complete the thought
- **Quotable endings** — if it sounds like a pull-quote, rewrite it. Readers smell manufactured profundity.
- **Binary contrasts** — "It's not X, it's Y" → state Y. This is the #1 AI tell on social.
- **Throat-clearing** — "Here's the thing:" / "Can we talk about" → cut, start with the point
- **Emphasis crutches** — "Let that sink in" / "Full stop" → delete. Trust the reader.
- **Three-item lists** — use two. Three is the AI default rhythm.
- **False agency** — "the market rewards", "the culture shifts" → name who does it
...
**Recommended: [Postiz](https://github.com/gitroomhq/postiz-app)** — open-source, self-hosted scheduler with support for 35+ platforms and a CLI + MCP + REST API for agent integration.

For X-only posting, any X/Twitter CLI or API client works.
